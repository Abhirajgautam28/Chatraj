...
const Project = () => {
  ...
  const getUserBubbleStyle = () => {
    ...
    const renderMessage = (msg) => {
        const isCurrentUser = msg.sender && typeof msg.sender === "object" && msg.sender._id && msg.sender._id.toString() === user._id.toString();
            ...
            <Box sx={{ display: 'flex', justifyContent: isCurrentUser ? 'flex-end' : 'flex-start', mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                    {!isCurrentUser && settings.display?.showAvatars && msg.sender && (
                        <Avatar>{msg.sender.firstName.charAt(0)}</Avatar>
                    )}
                    <Paper
                        elevation={2}
                        sx={{
                    ...
                    {isCurrentUser && settings.display?.showAvatars && (
  ...
  return (
    <Grid container sx={{ height: '100vh', overflow: 'hidden' }}>
      <Grid item xs={12} md={4} lg={3} sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRight: 1, borderColor: 'divider' }}>
      <section className="relative flex flex-col h-screen left min-w-96 bg-slate-100 dark:bg-gray-800">
        <AppBar position="static" color="default" elevation={1}>
          <Toolbar>
            ...
            {settings.display?.aiAssistant && (
              <IconButton title={t('aiAssistant')} onClick={() => setIsAIModalOpen(true)}>
                <SmartToyIcon />
              </IconButton>
            )}
            <Box sx={{ flexGrow: 1 }} />
            <IconButton onClick={() => setIsSettingsOpen(true)} title={t('settings')}>
              <SettingsIcon />
            </IconButton>
            <IconButton onClick={() => setShowSearch(!showSearch)}>
              <SearchIcon />
            </IconButton>
            <IconButton onClick={() => setIsSidePanelOpen(!isSidePanelOpen)}>
              <PeopleIcon />
            </IconButton>
          </Toolbar>
        </AppBar>
        <Box ref={messageBox} sx={{ flexGrow: 1, overflowY: 'auto', p: 2 }}>
            <List>
                {Object.keys(groupedMessages).sort((a, b) => a.localeCompare(b)).map((groupLabel) => (
                    <React.Fragment key={groupLabel}>
                        ...
                        {groupedMessages[groupLabel].map((msg, idx) => (
                            <ListItem key={msg._id ? `${groupLabel}-${msg._id}` : `${groupLabel}-idx-${idx}`} sx={{p:0}}>
                                {renderMessage(msg)}
                            </ListItem>
                        ))}
                    </React.Fragment>
        ...
        <Drawer anchor="left" open={isSidePanelOpen} onClose={() => setIsSidePanelOpen(false)}>
            ...
            <List>
              {project.users &&
                project.users.map((u) => (
                  <ListItem key={u._id}>
                    <ListItemAvatar>
                      <Avatar>{u.firstName.charAt(0)}</Avatar>
                    </ListItemAvatar>
      ...
      <section className="flex flex-grow h-full bg-blue-50 dark:bg-gray-900 right">
        ...
        <AppBar position="static" color="default" elevation={0}>
          <Toolbar>
            <Tabs value={currentFile} onChange={(e, newValue) => setCurrentFile(newValue)} variant="scrollable" scrollButtons="auto">
              {openFiles.map((file) => (
                <Tab key={file} label={file} value={file} />
              ))}
            </Tabs>
            <Box sx={{ flexGrow: 1 }} />
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} fullWidth maxWidth="xs">
          ...
          <List>
            {users.map((u) => (
              <ListItem
                ...
                selected={selectedUserId.has(u._id)}
              >
                <ListItemAvatar>
                  <Avatar>{u.firstName.charAt(0)}</Avatar>
                </ListItemAvatar>
        ...
        <DialogActions>
          ...
          <Button onClick={addCollaborators} variant="contained">
            {t('addCollaborators')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          ...
          >
            ...
            <Tab label="Language" value="accessibility" />
      ...
      <Dialog open={isAIModalOpen} onClose={() => setIsAIModalOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>
          AI Assistant
          <IconButton
            aria-label="close"
