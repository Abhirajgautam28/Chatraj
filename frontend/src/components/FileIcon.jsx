...
// Move icon maps outside the component to avoid re-creation on every render
const ICON_MAP = {
  // Web Development
  html: 'ri-html5-line',
  css: 'ri-css3-line',
...
const FileIcon = React.memo(({ fileName }) => {
  const getIcon = (name) => {
    ...
    // Check O(1) for special exact matches first
    if (SPECIAL_FILES[lowerName]) return SPECIAL_FILES[lowerName];

    // Check partial matches only if exact didn't work (still O(N) but restricted)
    for (const [special, icon] of Object.entries(SPECIAL_FILES)) {
      if (lowerName.includes(special)) return icon;
    }
